...
class Serial:
	...
	def __init__(self, task_strct=None):
		...
		if task_strct == None:
			...
			for line_n in range(1, self.task_num+1):
				ch = random.choice(task_list)
				line.append(ch)
				task_list.remove(ch)
				for column_n in range(1, self.proc_num+1):
	...
	def inherit(self,anthr_ser):
		...
		if(round(random.random()) == 0):
			...
			for line_num in range(1, self.task_num+1):
	...
	def mutate_q(self):
		...
		first_q = random.randrange(1, self.task_num+1)
		second_q = random.randrange(1, self.task_num+1)

		while (second_q == first_q) or (self.task_num==1):
			second_q = random.randrange(1, self.task_num+1)
	...
	def mutate_a(self):
		...
		first_a = random.randrange(1, self.task_num+1)
		second_a = random.randrange(1, self.task_num+1)

		while (second_a == first_a) or (self.task_num==1):
			second_a = random.randrange(1, self.task_num+1)
